...
class BtcDescManager {
 public:
  ...
  std::string loadBtcDescList(
    const std::string& file_path,
    std::vector<std::vector<BTC>>& btc_descs_list);
  void saveBtcDescList(
    const std::vector<std::vector<BTC>>& btc_descs_list,
    const std::string& seq_key, 
    const std::string& save_path);
  void SavePlaneClouds(const std::string& save_dir, std::vector<std::vector<BTC>> btc_descs_list); 
  bool LoadSinglePlaneCloud(const std::string& load_dir, int frame_index); 

  std::string HexStr2BinStr(const std::string& hex_str) {
      static const std::unordered_map<char, std::string> bin_dict = {
          {'0', "0000"}, {'1', "0001"}, {'2', "0010"}, {'3', "0011"},
          {'4', "0100"}, {'5', "0101"}, {'6', "0110"}, {'7', "0111"},
          {'8', "1000"}, {'9', "1001"}, {'A', "1010"}, {'B', "1011"},
          {'C', "1100"}, {'D', "1101"}, {'E', "1110"}, {'F', "1111"}};
      std::string bin_str;
      bin_str.reserve(hex_str.size() * 4);
      for (int i = 0; i < hex_str.size(); i++) {
        bin_str += bin_dict.at(hex_str[i]);
      }
    
      return bin_str;
  };
  void HexStr2BoolVec(const std::string& hex_str, std::vector<bool>& occ_vec,
      int length = -1) {
          if (length != -1)
              occ_vec.resize(length);
          else
              occ_vec.resize(hex_str.size() * 4);
  
              std::string bin_str = HexStr2BinStr(hex_str);
              for (int i = 0; i < occ_vec.size(); i++) {
              occ_vec[i] = bin_str[i] == '0' ? false : true;
              }
  };
  std::string BoolVec2HexStr(const std::vector<bool>& occupy_vec) {
      static const std::string hex_dict = "0123456789ABCDEF";
    
      std::string hex_str;
      hex_str.reserve(occupy_vec.size() / 4 + 1);
    
      for (int i = 0; i < occupy_vec.size() / 4; i++) {
        int num = 0;
        for (int j = i * 4; j < (i + 1) * 4; j++) {
          num = num << 1;
          if (occupy_vec[j]) num += 1;
        }
    
        hex_str.push_back(hex_dict[num]);
      }
    
      if (occupy_vec.size() % 4 != 0) {
        int num = 0;
        for (int i = occupy_vec.size() / 4 * 4; i < occupy_vec.size(); i++) {
          num = num << 1;
          if (occupy_vec[i]) num += 1;
        }
        hex_str.push_back(hex_dict[num]);
      }
